...
/* ============ Firebase cloud sync ============ */
// Paste your Firebase project's config object here (from Firebase Console >
// Project settings > Your apps). Leave firebaseConfig as null to keep the
// app fully local/offline with no sync.
const firebaseConfig = {
  apiKey: "AIzaSyCbdPfuT0AIthKi0YOr93m3qbzTONIEXVU",
  authDomain: "cagayan-attendance-system.firebaseapp.com",
  projectId: "cagayan-attendance-system",
  storageBucket: "cagayan-attendance-system.firebasestorage.app",
  messagingSenderId: "68651067721",
  appId: "1:68651067721:web:06362673225d517f365298"
};

let _db = null;
function cloudSyncEnabled(){
  return !!firebaseConfig && typeof firebase !== "undefined";
}
function getDb_(){
...
async function cloudSaveRecord(record){
  ...
  try {
    const db = getDb_();
    await db.collection("records").doc(String(record.id)).set(record);
...
async function cloudDeleteDateKey(dateKey){
  ...
  try {
    const db = getDb_();
    const snap = await db.collection("records").where("dateKey", "==", dateKey).get();
